"""60-Day Backtest - Balanced Settings (not too aggressive)

Key insight from previous test:
- 5% risk + 4 ATR TP = too aggressive, 62 stop losses
- Need balance between position size and stop distance

Try: 3% risk, NO fixed TP, ONLY trailing stop (8 ATR activation)
This lets winners run indefinitely until trailing stop triggers

Usage: uv run python scripts/backtest/balanced_60day_backtest.py
"""
...
async def main():
    ...
    print("\nRunning 60-day backtest with BALANCED settings...")
    ...
    print(f"\n{'='*60}")
    print("60-DAY BACKTEST - BALANCED SETTINGS")
    print(f"{'='*60}")
    ...
    output_file = Path(f"results/balanced_60day_{date_str}.md")

    report = f"""# 60-Day Backtest - BALANCED Settings
