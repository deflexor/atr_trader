"""Run 90-day backtest with enhanced signals across all 4 assets."""
...
async def main():
    ...
    enh_config = EnhancedSignalConfig(
    ...
    for sym in symbols:
        ...
        print(f'{sym}: {len(filtered)} candles')

        async def enh_gen(symbol, cs):
            return generate_enhanced_signal(symbol, cs, enh_config)
        ...
        print(f'  {len(trades)} trades ({longs}L/{shorts}S), {ret:+.2f}%, {elapsed:.0f}s')

    monthly = total_return / 3
    print(f'')
    print(f'=== 90-Day Enhanced Signals Results ===')
